...
router.get('/', async (req, res) => {
	...
	const Feeds = rawFeeds.items.map((item) => {
		const { content, contentSnipet, guild, isoDate, link, pubDate, title } = item;
	...
	const storeFeed = [ ...currFeeds, ...newFeeds ];
	...
	if (result) {
		res.render('board', { feeds: finalRes, categories });
	}
});
